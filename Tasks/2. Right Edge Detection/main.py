...
json_path = os.path.join(current_dir, 'right_edge.json')
csv_path = os.path.join(current_dir, 'right_edge.csv')
...
def movement():
    #print("here")
    global regions_, mynode_, right_edge
    regions = regions_

    front_right_region = np.min([regions_['fright']])
    back_right_region = np.min([regions_['bright']])

    print("Min distance in front region: ", front_right_region)
    print("Min distance in back region: ", back_right_region)

    #create an object of twist class, used to express the linear and angular velocity of the turtlebot 
    msg = Twist()
     
    right_edge.set_input('RFS', regions_['fright'])
    right_edge.set_input('RBS', regions_['bright'])
    right_edge() 
    output = right_edge.get_outputs() 
